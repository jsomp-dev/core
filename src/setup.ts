 ...
 * Cached at module level to ensure concurrent calls share the same initialization.
 */
let _initPromise: Promise<IJsompService> | null = null;

/**
 * Initialize JSOMP with optional configuration.
 * Uses a shared promise to handle concurrent calls (e.g., React StrictMode double-mount).
 * If already setup, returns the existing service directly.
 *
...
export const setupJsomp = async (config: JsompConfig = {}): Promise<IJsompService> => {
  // If already setup, return the existing service directly
  if (jsompEnv.isSetup) {
    return jsompEnv.service!;
  }

  // If already initializing, wait for the same promise instead of starting another
  if (_initPromise) {
    return _initPromise;
  }
  ...
  _initPromise = (async () => {
    ...
    if (shouldRegister('standard-pull', PipelineStage.PreProcess)) {
      const {pullPlugin} = await import('./engine/compiler/plugins/PullPlugin');
